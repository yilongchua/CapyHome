...
This skill uses local, keyless TTS via `assets/tts.py`:
- Default backend: macOS `say`
- Optional backend: XTTS v2 (if configured)
- Requires `ffmpeg` (to stitch segments and encode the final MP3)

Optional environment variables:
- `PODCAST_TTS_VOICE_MALE` (default: `Alex`)
- `PODCAST_TTS_VOICE_FEMALE` (default: `Samantha`)
- `XTTS_SPEAKER_WAV` (optional, enables XTTS v2 if it points to an existing wav)
- `XTTS_LANGUAGE` (optional, default: `en`)

If `XTTS_SPEAKER_WAV` is not set, the skill will default to `assets/max_verstappen_final.wav` if that file exists.
