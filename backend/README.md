...
| **Built-in** | `present_files`, `ask_user_for_clarification`, `view_image`, `task` (subagent) |
| **Community** | SearXNG (local web search), crawl4ai, SearXNG image search, optional Onyx bridge (non-production) |
...
- Optional: set `CAPYBARA_API_KEY_HEADER` (default: `x-api-key`).
...
```json
{
  "mcpServers": {
    "onyx": {
      "enabled": true,
      "type": "http",
      "url": "$ONYX_MCP_BASE_URL",
      "headers": {"Authorization": "$ONYX_MCP_AUTH_HEADER"}
    },
    "github": {
      "enabled": true,
      "type": "stdio",
...
- `CAPYBARA_HOME_CONFIG_PATH` - Override config.yaml location
- `CAPYBARA_HOME_EXTENSIONS_CONFIG_PATH` - Override extensions_config.json location
- Model API keys: `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `DEEPSEEK_API_KEY`, etc.
- Tool API keys: `TAVILY_API_KEY`, `GITHUB_TOKEN`, etc.
- Local model endpoint: `LOCAL_LLM_BASE_URL` (for local-only deployments)
...
- **SearXNG** / **crawl4ai** - Search, extraction, and crawling
...
- [Onyx Integration Blueprint](../docs/ONYX_CAPYBARA_INTEGRATION_BLUEPRINT.md)
