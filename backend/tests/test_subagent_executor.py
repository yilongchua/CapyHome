...
# -----------------------------------------------------------------------------
# P7: task-tool exclusion assertion tests
# -----------------------------------------------------------------------------


class TestTaskToolExclusion:
    """SubagentExecutor must raise if the 'task' tool leaks into the filtered tool list."""

    def test_raises_when_task_tool_present(self, classes):
        """Instantiating SubagentExecutor with a 'task' tool in the list must raise RuntimeError."""
        SubagentExecutor = classes["SubagentExecutor"]
        ...
        config = SubagentConfig(
            ...
            description="test",
            system_prompt="",
            tools=None,         # no allowlist, so all tools pass
            disallowed_tools=None,  # no denylist — 'task' will not be filtered out
            model="inherit",
            max_turns=10,
            timeout_seconds=60,
        )

        # Build a minimal mock BaseTool with name == "task"
        task_tool = MagicMock()
        task_tool.name = "task"

        with pytest.raises(RuntimeError, match="task"):
            SubagentExecutor(config=config, tools=[task_tool])

    def test_no_raise_when_task_tool_absent(self, classes):
        """No error when 'task' is not in the filtered tool list."""
        SubagentExecutor = classes["SubagentExecutor"]
        SubagentConfig = classes["SubagentConfig"]
        ...
        config = SubagentConfig(
            ...
            system_prompt="",
            tools=None,
            disallowed_tools=["task"],  # explicitly excluded
            model="inherit",
            max_turns=10,
            timeout_seconds=60,
        )
        ...
        # Should not raise — 'task' filtered out by disallowed_tools
        executor = SubagentExecutor(config=config, tools=[task_tool, bash_tool])
