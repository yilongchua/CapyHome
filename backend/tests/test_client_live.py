...
They are skipped in CI and must be run explicitly:
    ...
    CAPYBARA_RUN_LIVE_TESTS=1 PYTHONPATH=. uv run pytest tests/test_client_live.py -v -s
"""
...
elif os.environ.get("CAPYBARA_RUN_LIVE_TESTS") != "1":
    _skip_reason = "Set CAPYBARA_RUN_LIVE_TESTS=1 to run live tests"
...
class TestLiveToolUse:
    ...
    def test_agent_uses_ls_tool(self, client):
        """Agent attempts to list a directory and returns a usable response.

        Live environments can differ (tool availability, MCP reachability, model behavior),
        so this test accepts either an explicit ls tool call or a direct textual listing.
        """
        ...
        # Always require a final assistant response.
        assert len(ai_events) >= 1
        final_text = str(ai_events[-1].data.get("content", "")).strip()
        assert final_text

        # Prefer explicit ls tool invocation when the runtime exposes it.
        if tc_events:
            first_name = tc_events[0].data["tool_calls"][0]["name"]
            assert first_name in {"ls", "bash"}
