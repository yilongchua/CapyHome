...
10. **ViewImageMiddleware** / **ToolErrorBoundaryMiddleware** / **RetryPolicyMiddleware** / **ModelTimeoutMiddleware** - Vision injection, tool-call error boundary, retries, and bounded model/tool calls. `ToolErrorBoundaryMiddleware` is the **outermost** tool-call wrapper (sorts `before={"retry"}`): it catches any tool exception that survives `RetryPolicyMiddleware` (e.g. an MCP `504`/transport error from `websearch_search`) and converts it into a recoverable `ToolMessage(status="error")` instead of letting it crash the whole run. It re-raises langgraph control-flow signals (`GraphBubbleUp`/`GraphInterrupt`/`ParentCommand`, `GraphRecursionError`) so interrupts keep working. This mirrors `ToolNode(handle_tool_errors=True)` but composes with the project's other `wrap_tool_call` middlewares.
11. **WebSearchCircuitBreakerMiddleware** / **ToolResultTruncationMiddleware** / **WebSearchIngestionMiddleware** - Search retry controls, tool-output caps, and the vault-ingestion-queue bridge for `websearch_search` results (inner to summary/truncation so it sees the raw markdown)
...
Three knobs decide how much of a `websearch_search` result the lead agent sees, applied on the response path `tool → web_search_ingestion → web_search_summary → tool_result_truncation`:
