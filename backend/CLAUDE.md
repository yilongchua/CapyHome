...
- **`internal_tools_work.json`** — full execution surface served when `mode=work` or `mode=auto` (and the default fallback when mode is unset). Includes everything in the plan file plus the execution tools, with descriptions framed for execution.
- **Legacy `internal_tools.json`** — combined file kept as a fallback when neither per-mode file exists. New edits should go into the per-mode files; the legacy file is no longer the primary source of truth.
- Each entry in either file is a `ToolDefinition` (schema in `src/tools/schema.py`) carrying `name`, `description`, `handler` (resolvable module:variable path), declarative filters (`mode`, `phase`, `groups`, `endpoint`, `requires_vision`, `requires_subagent_enabled`), a `returns` blurb, `examples`, and a JSON-Schema `parameters` block. The `name` must match the decorated tool's `name`. The `mode` field within each entry is now redundant for file selection (the file name already encodes mode) but stays for runtime middleware filtering.
