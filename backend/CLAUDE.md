...
- **Turn budgets**: `knowledge-researcher` and `comparison-dimension-researcher` default to `max_turns=25`, which becomes the subagent run's LangGraph `recursion_limit`. `max_turns` is overridable from the main config via `subagents.agents.<name>.max_turns` (applied in `registry.get_subagent_config` alongside the existing `timeout_seconds` override); the plan-mode helpers source theirs there (`scope-researcher: 12`, `finder-agent: 15`)
