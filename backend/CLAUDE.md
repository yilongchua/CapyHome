...
- **`internal_tools_plan.json`** — read-only / planning-oriented surface served when `mode=plan`. Excludes write/execute tools (`bash`, `write_file`, `str_replace`); descriptions frame each tool in terms of drafting and information gathering. Includes `grep` (read-only search) and a plan-scoped `task` whose `subagent_type` enum offers only the read-only planning subagents (`scope-researcher`, `finder-agent`) — spawning is further enforced by `SubagentConfig.modes`.
