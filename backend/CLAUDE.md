...
- **`internal_tools_plan.json`** — read-only / planning-oriented surface served when `mode=plan`. Excludes execution tools (`bash`, `write_file`, `str_replace`, `task`); descriptions frame each tool in terms of drafting and information gathering.
