...
11. **WebSearchCircuitBreakerMiddleware** / **ToolResultTruncationMiddleware** / **WebSearchIngestionMiddleware** / **SubagentLimitMiddleware** - Search retry controls, tool-output caps, vault-ingestion-queue bridge for `websearch.search` results (inner to summary/truncation so it sees the raw markdown), and endpoint-aware `task` scheduling
