...
│   ├── src/
│   │   ├── agents/            # LangGraph agent system (two graphs sharing one state schema)
│   │   │   ├── work_agent/    # Work-mode agent (factory + system prompt)
│   │   │   ├── plan_agent/    # Plan-mode agent (factory + plan-mode prompt overlay)
│   │   │   ├── common/        # Shared utilities: middleware_registry, mode resolver, handoff serializer
...
**Work Agent** (`src/agents/work_agent/agent.py`):
- Entry point: `make_work_agent(config: RunnableConfig)` registered in `langgraph.json` as graph `work_agent`
- Dynamic model selection via `create_chat_model()` with thinking/vision support
- Tools loaded via `get_available_tools()` - combines sandbox, built-in, MCP, community, and subagent tools
- System prompt generated by `apply_prompt_template()` with skills, memory, and subagent instructions
- Accepts an optional `prompt_template_fn` so `make_plan_agent` can inject the plan-mode prompt without duplicating model/tool/middleware setup

**Plan Agent** (`src/agents/plan_agent/agent.py`):
- Entry point: `make_plan_agent(config: RunnableConfig)` registered in `langgraph.json` as graph `plan_agent`
- Forces `current_mode="plan"` in configurable, then delegates to `make_work_agent(..., prompt_template_fn=plan_apply_prompt_template)` — the middleware registry conditionally activates plan-mode middlewares (`PlannerMiddleware`, `PlanEvaluatorMiddleware`, `PlanFileSyncMiddleware`, `TodoDagMiddleware`) when `is_plan_mode=True`
- Owns `PLAN_MODE_SECTION` and `PLAN_BACKGROUND_FOLLOWUP_SECTION`; work-mode prompt is now free of plan-mode content
- **Planner-as-tool (B++)**: plan authoring is done by the agent itself, not a hidden one-shot LLM call. The plan_agent investigates with read-only tools and then calls the **`write_plan` tool** ([src/tools/builtins/write_plan_tool.py](src/tools/builtins/write_plan_tool.py)) to emit the canonical `plan.md` + `plan`/`todo_graph` state. The tool's arguments **are** the plan contract (title/objective/summary/domain/todos/clarifications…), so determinism lives in the schema + handler (which runs `normalize_todo_nodes` → DAG ready-set → `serialize_plan_md` → `plan_created` SSE), not in a separate model. This replaced the old design where `PlannerMiddleware` made a blind toolless one-shot call and `jump_to=end` before the conversational agent ever ran — two competing identities/plan authors. `PlannerMiddleware` is now a thin orchestrator: it observes the `plan_just_written` flag the tool sets, spawns the work-mode handoff when approved+foreground, halts the planning turn, and advances inline clarifications.
- **Self-contained prompt**: `plan_agent/prompt.py` no longer imports the work-agent prompt. It carries its own copy of the operational base sections (thinking style, clarification, working directory, fetch policy, etc.) with **no `<role>` block**. Plan-mode identity lives solely in the `<identity>` block of `PLAN_MODE_SECTION` (one identity — "CapyHome's Plan-Mode strategist"), and `<plan_mode>` instructs the agent to investigate, optionally ask via `ask_user_for_clarification`, then call `write_plan` as its terminal action.
- **Plan-mode finder tier (parallel scope discovery)**: when `subagent_enabled=True`, the plan prompt injects a `<subagent_system>` block describing two read-only planning subagents the planner can dispatch (in parallel) via `task`: `scope-researcher` (web + vault scope understanding) and `finder-agent` (local file/repo mapping). They investigate and return briefs; they never execute work or write the deliverable. The plan tool catalog exposes `task` for these, and `SubagentConfig.modes` gating (see Subagent System) ensures only plan-allowed subagents can be spawned in Plan Mode.
- Entry to Plan Mode is fully user-initiated via the UI (Shift+Tab). Work Mode never auto-escalates into Plan Mode — both the complexity-based escalation and the auto-mode plan-adaptation respawn (`_spawn_plan_rerun` / `_MAX_AUTO_ADAPTATION_ATTEMPTS`) have been removed. When a plan stalls (no ready todos but pending ones remain), `WorkModeMiddleware._handle_plan_adapted` emits a `plan_adapted` SSE event so the UI can prompt the user to switch to Plan Mode; the user decides whether to revise.

**Mode resolution** (`src/agents/common/mode.py`):
- Canonical field is `current_mode: Literal["work", "plan"]` in `config.configurable`
- `resolve_current_mode(cfg)` accepts canonical `current_mode`, legacy `mode` string, or legacy `is_plan_mode` boolean (writers dual-write all three during transition)

**Canonical plan.md handoff** (`src/agents/common/handoff.py`):
- `serialize_plan_md(plan, todo_graph, body_renderer=...)` writes `plan_version: 5` frontmatter with structured todos + a human-readable body
- `parse_plan_md(text)` reads canonical frontmatter back into `(plan, todo_graph)`; returns `None` for legacy v4 plans so callers fall back to checkpointed state
...
- Extends `AgentState` with: `sandbox`, `thread_data`, `title`, `artifacts`, `todos`, `todo_graph`, `plan`, `eval_attempts`, `handoff_artifacts`, `retry_meta`, `hooks_state`, `uploaded_files`, `viewed_images`, `trajectory`, `skill_disclosure`, `resume_meta`, `scratchpad`, `task_memory`, `memory_version_ref`
...
Middlewares execute in strict order in `src/agents/work_agent/agent.py`:
...
5. **WorkModeMiddleware** / **PlanExecutionGateMiddleware** - Work-mode execution and draft-plan approval gating. `PlanExecutionGateMiddleware` is registered in both graphs but only acts in **Work Mode**: it blocks execution tools (`write_file`, `bash`, `web_search`, `task`, …) while a `plan` in state is still `status: draft` — until the user approves via the Execute Plan action (auto-mode marks plans `approved` up-front, so it is unaffected). It also blocks execution while `clarification_pending` is set. `write_todos`/`recall`/`ask_user_for_clarification` and read-only tools stay allowed when draft. **`/recover` is exempt** — the `recover_todo_command` runtime-context flag (set by the frontend recover action) is an explicit "finish the plan" instruction, so the gate bypasses entirely and lets the run complete the incomplete todos. Plan Mode itself is **not** gated here — the per-mode tool catalog (`internal_tools_plan.json`) already restricts that surface and intentionally exposes `web_search`/`grep`/finder subagents.
...
8. **PlannerMiddleware** / **PlanEvaluatorMiddleware** / **TodoDagMiddleware/TodoListMiddleware** - Plan-turn orchestration, plan checks, and todo graph tracking. `PlannerMiddleware` no longer generates plans (that moved to the `write_plan` tool — see Plan Agent above); it only halts the planning turn after a plan is authored (`plan_just_written` flag → `jump_to=end`), spawns the work-mode handoff when approved+foreground, and advances inline clarifications. `PlanEvaluatorMiddleware` runs a deterministic pre-check (cycle detection short-circuits the LLM; dangling deps, duplicate IDs, and non-`todo-N` IDs are auto-repaired) before prompting the planner model. The prompt injects `plan.domain`, `plan.acceptance_criteria`, and rich todo fields (`objective`, `failure_fallback`, `steps[].completion_requirement`) read from `plan["todos"]`. The LLM contract is `{ok, issues, advice, patch}` where `patch` is a list of `{op: modify|add|remove, ...}` ops applied via `merge_todo_nodes`; modifications that strip rich fields are rejected. Legacy `revised_todos` (full replacement) is still accepted. The loop re-evaluates patched plans up to `evaluator.max_attempts`, emitting `decision=max_attempts_reached` when the cap is hit. `eval_attempts` is tracked in `plan_eval_attempts` (distinct from the legacy `EvaluatorMiddleware` counter).
...
10. **ViewImageMiddleware** / **ToolErrorBoundaryMiddleware** / **RetryPolicyMiddleware** / **ModelTimeoutMiddleware** - Vision injection, tool-call error boundary, retries, and bounded model/tool calls. `ToolErrorBoundaryMiddleware` is the **outermost** tool-call wrapper (sorts `before={"retry"}`): it catches any tool exception that survives `RetryPolicyMiddleware` (e.g. an MCP `504`/transport error from `websearch.search`) and converts it into a recoverable `ToolMessage(status="error")` instead of letting it crash the whole run. It re-raises langgraph control-flow signals (`GraphBubbleUp`/`GraphInterrupt`/`ParentCommand`, `GraphRecursionError`) so interrupts keep working. This mirrors `ToolNode(handle_tool_errors=True)` but composes with the project's other `wrap_tool_call` middlewares.
11. **WebSearchCircuitBreakerMiddleware** / **ToolResultTruncationMiddleware** / **WebSearchIngestionMiddleware** - Search retry controls, tool-output caps, and the vault-ingestion-queue bridge for `websearch.search` results (inner to summary/truncation so it sees the raw markdown)
...
- `grep` - Recursive regex content search (read-only; skips binary + noise dirs; returns `path:line:text`, capped by `max_results`). Available in **both** plan and work catalogs.
...
When `app_config.json_driven_tools` is `true` (the default), the LLM-facing contract for built-in and sandbox tools is sourced from one of two per-mode catalog files. The Python handlers (`@tool`-decorated functions in `src/tools/builtins/` and `src/sandbox/tools.py`) continue to provide the runtime behavior, but their LLM-visible `description` and per-argument descriptions are overridden from JSON. This lets a shared tool (e.g. `read_file`, `write_todos`) carry slightly different framing for Plan Mode vs Work Mode without coupling the two surfaces, and collapses an audit surface that previously lived across 13+ files into two declarative files.

- **`internal_tools_plan.json`** — read-only / planning-oriented surface served when `mode=plan`. Excludes write/execute tools (`bash`, `write_file`, `str_replace`); descriptions frame each tool in terms of investigation and plan authoring. Includes `grep` (read-only search), a plan-scoped `task` whose `subagent_type` enum offers only the read-only planning subagents (`scope-researcher`, `finder-agent`), and **`write_plan`** — the terminal plan-authoring tool whose arguments are the full plan contract (the plan_agent calls it once after investigating). Spawning is further enforced by `SubagentConfig.modes`.
- **`internal_tools_work.json`** — full execution surface served when `mode=work` or `mode=auto` (and the default when mode is unset). Includes everything in the plan file plus the execution tools, with descriptions framed for execution.
- **Per-mode resolution is up-front, not runtime** — `get_available_tools(mode=...)` in `src/tools/tools.py` picks the right catalog file at agent build time. Plan-mode agents and work-mode agents are different LangGraph graphs (`make_plan_agent` vs `make_work_agent` in `langgraph.json`); plan-status transitions (draft → approved) are inter-graph (plan_agent terminates, work_agent spawns), so no runtime mode/phase filter is needed.
- **Community-tool mode scoping** — community tools (`web_search`, `query_knowledge_vault`, `save_to_knowledge_vault`) have no JSON catalog entry, so `_COMMUNITY_TOOL_MODES` in `src/tools/tools.py` mode-scopes them at load time alongside the catalog selection. `web_search` is available in all modes; the knowledge-vault tools are work-only. (The legacy `scope_search` wrapper is deprecated — `web_search` is exposed directly in Plan Mode.)
- Each entry is a `ToolDefinition` (schema in `src/tools/schema.py`) carrying `name`, `description`, `handler` (resolvable module:variable path), declarative filters (`mode`, `phase`, `groups`, `endpoint`, `requires_vision`, `requires_subagent_enabled`), a `returns` blurb, `examples`, and a JSON-Schema `parameters` block. The `name` must match the decorated tool's `name`. The `mode` and `phase` fields remain on each entry for the audit tool and diagnostics but are no longer re-checked at runtime.
...
- **Loader** (`src/tools/loader.py`) — `build_structured_tool(defn)` resolves the handler and mutates its `description`/`args_schema`; the policy is attached via `_capyhome_policy` so the audit tool can read it. `filter_mcp_tools_by_policy(...)` enforces MCP server policy.
- **`PhaseToolFilterMiddleware`** — kept, but its only job now is a first-turn execution-tool gate: in Work Mode, when there is no plan yet and no prior AI message, execution tools (bash/write_file/task/web_search/...) are hidden so the LLM has to reason before acting. From turn 2 onward the full catalog is exposed. Mode and plan-status filtering have been removed (they are resolved up-front by catalog selection).
- **Drift validator** — `tests/test_tool_schema_sync.py` walks every JSON entry across BOTH catalog files, instantiates the handler, and asserts JSON parameters match the handler signature (with `tool_call_id` / `runtime` injection params excluded). It also asserts that the plan catalog excludes write/execute tools (`bash`/`write_file`/`str_replace`) while the work catalog includes them, and that the plan-mode `task` enum offers only the planning subagents (`scope-researcher`, `finder-agent`). Failures in CI mean rename one side or the other.
- **Audit tool** — `make tools-audit` (or `python -m src.tools.audit --mode work --phase approved` / `--mode plan`) prints a Markdown table of the resolved catalog for a given mode/phase/vision/subagent triple, picking the per-mode catalog file automatically. Use it to review JSON edits before merging.
...
- **Token streaming on by default**: `merged.setdefault("streaming", True)` so LangGraph forwards incremental message/thinking deltas to the frontend (time-to-first-token) instead of emitting only a full message at node completion. Applies to both `config.yaml` models and synthesized user-endpoint models. Override per-model with `streaming: false` in the model config (`ModelConfig` is `extra="allow"`) or via caller kwargs.
...
### Search-results ingestion queue lifecycle (`vault_learning/_queue.py`)

Enriched web-search results are appended to `03_ops/queues/search_results_ingestion_queue.json` and flow `queued → claimed → ingested|rejected`.

**How results reach the queue:** historically the in-backend `web_search` community tool called `enqueue_search_results` directly ([web_search/tools.py](src/community/web_search/tools.py)). After web search was migrated out to the standalone `websearch.search` MCP server (`http://localhost:9000/mcp`), that external service has **no coupling to the vault**, so the enqueue is now performed by `WebSearchIngestionMiddleware` ([web_search_ingestion_middleware.py](src/agents/middlewares/web_search_ingestion_middleware.py)). It observes the `websearch.search` tool result as it passes back through the backend, re-renders each result via `web_search.tools._render_result_markdown`, and calls `enqueue_search_results` (rows carry `source_tool: "websearch.search"`). It is registered `after={"web_search_summary"}` so it runs **inner** to the summary/truncation middlewares — capturing the raw, full markdown before either can replace or truncate it (LangChain composes `wrap_tool_call` first→outermost; response flows tool→last→first). The middleware is gated on `knowledge_vault.enabled && knowledge_vault.search_results_queue_enabled` and is a pure side effect (the `ToolMessage` is passed through unchanged).

Terminal rows are **removed**, not just status-flipped:

- **`ingested` rows are purged at the end of every `ingest()` run** via `purge_ingested_queue_items()` ([_ingest.py](src/control_plane/vault_learning/_ingest.py), reported as `purged_ingested_count`). This is safe because the durable dedup record is `manifest["sources"][source_id].hash_history`, consulted at ingest time — a re-queued duplicate is caught there and returns `skipped_unchanged`, never a duplicate vault source. Consequently the enqueue dedup guard only checks `{queued, claimed}` (not `ingested`).
- **`rejected` rows are aged out (default 72h) by both the ingest run and the lint pass.** `ingest()` and `lint_vault()` each call `purge_aged_rejected_queue_items()`, dropping rows older than `search_results_rejected_retention_hours` (reported as `purged_rejected_count`). Lint covers the case where the ingest loop is idle.
- **"Run ingest now" always cleans up, even with nothing to ingest.** `start_vault_ingest_job()`'s drain loop returns early when no item is claimable, so `ingest()` (and its per-run purge) may never fire. To guarantee the button flushes terminal rows regardless, worker 0 runs a job-level housekeeping tail (Phase 1.6 in [service.py](src/control_plane/service.py)) that calls both `purge_ingested_queue_items()` and `purge_aged_rejected_queue_items()` once per non-cancelled job. It is peer-safe — only terminal rows are removed, never another worker's `claimed` items.
- **`_trim_queue` ([_base.py](src/control_plane/vault_learning/_base.py)) remains a safety net** — a hard cap (`search_results_max_queue_items`, 5000) plus age-out (`search_results_terminal_retention_hours`, 168h) applied on every `_save_queue`. All purge paths and the trim share the `_queue_row_ts` timestamp helper.
...
### Web-search result fidelity (what reaches the lead agent)

Three knobs decide how much of a `websearch.search` result the lead agent sees, applied on the response path `tool → web_search_ingestion → web_search_summary → tool_result_truncation`:

1. `web_search_summary.summary_threshold_chars` (default **12000**) — results larger than this are replaced by a ≤250-word summary; smaller results pass through in full.
2. `routing.timeouts.tool_result_caps["web_search"]` (default **12000**) — hard char cap on the (possibly summarized) result.
3. `routing.timeouts.unsummarized_web_search_chars` (default **12000**) — *adaptive* cap applied by `ToolResultTruncationMiddleware` when a web_search result lacks the `[Summarized by web_search_summary_middleware` marker.

**These three must move together** (hybrid policy). They are aligned at 12000 so a result that fits the cap reaches the agent in full and is *not* chopped, while larger results are summarized. Raising only the summary threshold backfires: un-summarized results between the old and new threshold would hit the adaptive cap (historically 3500) and be truncated harder than the summary would have been. The adaptive cap exists because synthesis drowns in multiple raw excerpts after concurrent searches (thread-cd90decb audit); setting it equal to the web_search cap disables the adaptive chop.

**Citations survive summarization.** Because the summary *replaces* the raw result, `_SUMMARY_PROMPT_TEMPLATE` ([web_search_summary_middleware.py](src/agents/middlewares/web_search_summary_middleware.py)) instructs the summarizer to embed inline `[citation:TITLE](URL)` per claim (URLs drawn from the raw results, never invented). The lead/plan agent prompts also carry a `CITATIONS_SECTION` (already wired into both `apply_prompt_template` section lists), so final answers cite sources in both modes regardless of whether the result was summarized.
...
- `stream(message, thread_id)` — yields `StreamEvent` aligned with LangGraph SSE protocol:
  ...
  - `"end"` — stream finished
- Agent created lazily via `create_agent()` + `_build_middlewares()`, same as `make_work_agent`
