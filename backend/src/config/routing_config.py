...
class RoutingTimeoutsConfig(BaseModel):
    ...
    tools: dict[str, int] = Field(
        ...
        description="Per-tool wall-clock timeout (seconds). Keys match the LLM-facing tool name. web_search now runs as MCP tool 'websearch_search', bounded by its per-server timeout_seconds; key it here only for an extra middleware cap.",
    )
