...
class SkillCurationRequest(BaseModel):
    """Request model for skill curation proposals."""

    limit: int = Field(default=5, ge=1, le=50, description="Maximum number of proposals to generate.")


class SkillCurationResponse(BaseModel):
    """Response model for skill curation proposals."""

    proposals: list[dict] = Field(default_factory=list, description="Generated skill proposal metadata.")
...
def _safe_extract_zip(zip_ref: zipfile.ZipFile, destination: Path) -> None:
    """Safely extract ZIP archive while preventing path traversal and symlink entries."""
    ...
    for member in zip_ref.infolist():
        ...
        if Path(member_name).is_absolute() or re.match(r"^[A-Za-z]:[\\/]", member_name):
            raise HTTPException(status_code=400, detail=f"Unsafe path in skill archive: {member_name}")

        # Reject symlink entries to avoid escaping extraction boundaries.
        unix_mode = (member.external_attr >> 16) & 0o170000
        is_symlink = unix_mode == 0o120000
        if is_symlink:
            raise HTTPException(status_code=400, detail=f"Symlink entries are not allowed in skill archive: {member_name}")
        ...
        if destination_resolved not in target_path.parents and target_path != destination_resolved:
            raise HTTPException(status_code=400, detail=f"Path traversal detected in skill archive: {member_name}")
...
@router.post(
    "/skills/curation/proposals",
    response_model=SkillCurationResponse,
    summary="Generate Skill Proposals",
    description="Generate proposal-only skills from trajectory usage evidence.",
)
async def generate_skill_curation_proposals(request: SkillCurationRequest) -> SkillCurationResponse:
    """Generate human-review-required skill proposals."""
    try:
        return SkillCurationResponse(proposals=generate_skill_proposals(limit=request.limit))
    except Exception as e:
        logger.error(f"Failed to generate skill proposals: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to generate skill proposals: {str(e)}")
