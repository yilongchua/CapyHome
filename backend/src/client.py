...
class CapyHomeClient:
    ...
    def _get_runnable_config(self, thread_id: str, **overrides) -> RunnableConfig:
        ...
        current_mode = "plan" if is_plan_mode else (mode or "work")
        configurable = {
            "thread_id": thread_id,
            "model_name": overrides.get("model_name", self._model_name),
            "thinking_enabled": overrides.get("thinking_enabled", self._thinking_enabled),
            "current_mode": current_mode,
            "is_plan_mode": is_plan_mode,  # legacy dual-write; remove after step 8
        ...
        configurable["mode"] = current_mode  # legacy alias; remove after step 8
    ...
    def _ensure_agent(self, config: RunnableConfig):
        ...
        from src.config.harness_config import get_harness_config  # noqa: PLC0415 — lazy import avoids circular dep

        cfg = config.get("configurable") or {}
        # Include harness.enabled in the cache key so the Gateway kill-switch
        # propagates to already-running embedded clients without an explicit
        # reset call — next invoke picks up the new middleware shape.
        key = (
            cfg.get("model_name"),
            cfg.get("thinking_enabled"),
            cfg.get("current_mode") or cfg.get("mode") or ("plan" if cfg.get("is_plan_mode") else "work"),
        ...
        router = ModelRouter(app_config=self._app_config)
        model_name = router.resolve("generator", requested_model=requested_model_name)
        ...
        kwargs: dict[str, Any] = {
            ...
            "system_prompt": (
                plan_apply_prompt_template
                if resolve_current_mode(cfg) == "plan"
        ...
        logger.info("Agent created: requested_model=%s resolved_model=%s thinking=%s", requested_model_name, model_name, thinking_enabled)
    ...
    def stream(
        ...
        **kwargs,
        ...
        current_mode_ctx = cfg.get("current_mode") or cfg.get("mode") or ("plan" if cfg.get("is_plan_mode") else "work")
        context = {
            "thread_id": thread_id,
            "model_name": resolved_model,
            "thinking_enabled": cfg.get("thinking_enabled", True),
            "current_mode": current_mode_ctx,
            "is_plan_mode": current_mode_ctx == "plan",  # legacy dual-write
            "mode": current_mode_ctx,  # legacy alias
    ...
    async def astream(
        ...
        **kwargs,
        ...
        Use this when configured tools are async-only. Event payloads match the
    ...
    def resume_run(
        ...
        **kwargs,
        ...
        """Resume a paused/interrupted run from existing checkpointed thread state.

        Args:
            thread_id: Thread ID used for persisted state.
            run_id: Caller-facing run id for trace metadata.
            resume_payload: Optional ``Command.resume`` payload.
            **kwargs: Same override keys as ``stream`` (model_name, thinking_enabled, etc.).

        Returns:
            Dict containing resume metadata and final text/state snapshot.
        """
        resume_cfg = get_resume_config()
        if not resume_cfg.enabled:
            raise ValueError("Resume is disabled by configuration.")
        ...
        if isinstance(resume_payload, dict):
            depth = int(resume_payload.get("resume_depth", 0))
            if depth >= resume_cfg.max_resume_depth:
                raise ValueError(f"Resume depth exceeds configured max ({resume_cfg.max_resume_depth}).")

        command = Command(resume=resume_payload if resume_payload is not None else {"run_id": run_id})

        last_text = ""
        final_values: dict[str, Any] = {}
        for chunk in self._agent.stream(command, config=config, context=context, stream_mode="values"):
    ...
    def update_mcp_config(self, mcp_servers: dict[str, dict]) -> dict:
        ...
        if config_path is None:
            raise FileNotFoundError("Cannot locate extensions_config.json. Set CAPYBARA_HOME_EXTENSIONS_CONFIG_PATH or ensure it exists in the project root.")
    ...
    def get_memory_config(self) -> dict:
        ...
        return {
            ...
            "global_scope_enabled": getattr(config, "global_scope_enabled", True),
            "workspace_scope_enabled": getattr(config, "workspace_scope_enabled", True),
    ...
    def get_memory_status(self) -> dict:
        ...
        return {
            ...
            "memory_version_ref": get_memory_version_reference(),
        }

    def list_memory_versions(self, limit: int = 50) -> dict:
        """List append-only memory versions."""
        from src.agents.memory.store import list_memory_versions

        return {"items": list_memory_versions(limit=limit)}

    def get_memory_version(self, version_id: str) -> dict | None:
        """Get a specific memory version record."""
        from src.agents.memory.store import get_memory_version

        return get_memory_version(version_id)
    ...
    def redact_memory(
        ...
        *,
        ...
        """Apply auditable redaction and persist as a new memory version."""
        from src.agents.memory.store import redact_memory

        return redact_memory(
            agent_name=None,
            fact_ids=fact_ids,
            pattern=pattern,
            reason=reason,
