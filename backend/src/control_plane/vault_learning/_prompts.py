...
exclusively consumed by ``AnalysisMixin._analyze_source``. Co-locating the
prompt with its single caller makes the contract easier to read and edit.

A single merged call now produces both the structured analysis *and* the one
page-body field that carries genuinely new content (``evidence_markdown``).
The other markdown sections (summary/claims/backlinks) are derived in code
from the analysis fields, so the model is never asked to re-emit them.
"""
...
- evidence_markdown: string (markdown bullet list of the most citable supporting
  evidence — concrete facts, figures, quotes, or findings drawn verbatim-ish from
  the source. This is the one field that should add detail beyond the summary.)
...
- evidence_markdown must use markdown bullets ("- ...") and stay faithful to the text.
