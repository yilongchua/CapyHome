...
def apply_prompt_template(
    ...
    *,
    ...
    plan_mode: bool | None = None,  # deprecated: no-op; use plan_agent.prompt.apply_prompt_template
) -> str:
    """Build the work-mode system prompt.

    ``plan_mode`` is accepted for backwards compatibility but ignored — plan-mode
    discipline lives in ``plan_agent.prompt.apply_prompt_template`` after the
    work/plan agent split. Pass ``background_followup`` independently if needed.
    """
    del plan_mode  # ignored; see docstring
    from src.agents.work_agent.prompt_cache import get_cached_prompt
    from src.config import get_app_config
