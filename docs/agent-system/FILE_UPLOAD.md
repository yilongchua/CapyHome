...
CapyHome 后端提供了完整的文件上传功能，支持多文件上传，并自动将 Office 文档和 PDF 转换为 Markdown 格式。
  ...
  "files": [
    {
      ...
      "size": 1234567,
      "path": ".capyhome/threads/{thread_id}/user-data/uploads/document.pdf",
      "virtual_path": "/mnt/user-data/workspace/uploads/document.pdf",
      "artifact_url": "/api/threads/{thread_id}/artifacts/mnt/user-data/workspace/uploads/document.pdf",
      "markdown_file": "document.md",
      "markdown_path": ".capyhome/threads/{thread_id}/user-data/uploads/document.md",
      "markdown_virtual_path": "/mnt/user-data/workspace/uploads/document.md",
      "markdown_artifact_url": "/api/threads/{thread_id}/artifacts/mnt/user-data/workspace/uploads/document.md"
    }
  ],
...
- document.pdf (1.2 MB)
  Path: /mnt/user-data/workspace/uploads/document.pdf

- document.md (45.3 KB)
  Path: /mnt/user-data/workspace/uploads/document.md
...
# 读取转换后的 Markdown（推荐）
read_file(path="/mnt/user-data/workspace/uploads/document.md")
```

**路径映射关系：**
- Agent 使用：`/mnt/user-data/workspace/uploads/document.pdf`（虚拟路径）
- 实际存储：`backend/.capyhome/threads/{thread_id}/user-data/uploads/document.pdf`
- 前端访问：`/api/threads/{thread_id}/artifacts/mnt/user-data/workspace/uploads/document.pdf`（HTTP URL）

上传流程采用“线程目录优先”策略：
- 先写入 `backend/.capyhome/threads/{thread_id}/user-data/uploads/` 作为权威存储
- 本地沙箱（`sandbox_id=local`）直接使用线程目录内容
- 非本地沙箱会额外同步到 `/mnt/user-data/workspace/uploads/*`，确保运行时可见
...
2. 检查 thread_id 是否正确
3. 确认文件确实已上传到 `backend/.capyhome/threads/{thread_id}/user-data/uploads/`
...
1. **文件预览**：添加预览端点，支持在浏览器中直接查看 file
2. **批量删除**：支持一次删除多个 file
3. **文件搜索**：支持按文件名或类型搜索
4. **版本控制**：保留 file 的多个版本
5. **压缩包支持**：自动解压 zip file
